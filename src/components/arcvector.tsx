...
export class ArcVector extends VectorBase {
    ...
    public center(): Vector2 {
        if (this.counter()) {
            const halfAngle = (this.from() - this.to()) * 0.5;
            return Vector2.fromDegrees(this.to() + halfAngle).scale(this.radius());
        } else {
            const halfAngle = (this.to() - this.from()) * 0.5;
            return Vector2.fromDegrees(this.from() + halfAngle).scale(this.radius());
        }
    }
